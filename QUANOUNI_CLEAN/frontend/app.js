    ...
    try {
        ...
        if (data.sources && data.sources.length) {
            data.sources.forEach(source => {
                ...
                const clickHandler = hasViewerData ? `onclick="openDocumentViewer('${source.document_id}', ${source.chunk_index})"` : '';
                const cursorStyle = hasViewerData ? 'cursor: pointer;' : 'cursor: default;';
                const viewBtn = hasViewerData ? `<button class="view-source-btn" style="margin-top: 0.5rem; padding: 5px 10px; font-size: 0.8em; background: var(--accent-color); color: white; border: none; border-radius: 5px; cursor: pointer;" ${clickHandler}>📖 عرض المصدر</button>` : '';

                html += `
                    <div class="source-card" style="${cursorStyle}" ${clickHandler}>
                        <div class="source-header">
